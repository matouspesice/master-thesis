...
Figure~\ref{fig:latency-pipeline} summarises the composition of motion-to-photon latency in a system such as the present one: from capture (sensor exposure, readout, transfer to host) through pose inference and optional filtering in Python, UDP transmission of keypoints, to Unity (parse, drive avatar, render) and finally display. Optimisation is applied at each stage: capture (camera choice, buffer size, Section~\ref{sec:sensing}), inference (model size, detector frequency, smoothing), and rendering (frame rate, minimal processing).

\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[
      node distance=0.5cm and 0.35cm,
      box/.style={rectangle, draw, rounded corners=1.5pt, minimum height=0.65cm, font=\small},
      small/.style={font=\footnotesize},
      arrow/.style={-{Stealth[length=1.8mm]}, thick}
    ]
    \node[box] (capture) {Capture};
    \node[box, right=of capture] (infer) {Pose infer.};
    \node[box, right=of infer] (udp) {UDP};
    \node[box, right=of udp] (unity) {Unity};
    \node[box, right=of unity] (disp) {Display};
    \draw[arrow] (capture) -- (infer);
    \draw[arrow] (infer) -- (udp);
    \draw[arrow] (udp) -- (unity);
    \draw[arrow] (unity) -- (disp);
    \node[below=0.15cm of capture, small, align=center] {optim.: camera, buffer};
    \node[below=0.15cm of infer, small, align=center] {optim.: model, det.\,freq.};
    \node[below=0.15cm of unity, small, align=center] {optim.: parse, avatar, render};
  \end{tikzpicture}
  \caption{Motion-to-photon latency pipeline: capture $\rightarrow$ pose inference (Python) $\rightarrow$ UDP $\rightarrow$ Unity (parse, drive avatar, render) $\rightarrow$ display. The text under each stage describes the main optimisation levers for that stage (e.g.\ camera choice and buffer size for capture; model size and detector frequency for inference; parsing, avatar update, and rendering for Unity).}
...
In VR, the delay between the user's motion and the update of the visual display (motion-to-photon latency) directly affects presence, motor performance, and comfort. Literature on VR and sensorimotor control often cites motion-to-photon latency below 50\,ms as ideal for immersive interaction, whereas latencies above 90--130\,ms can degrade user performance \cite{caserman2019latency, warburton2023motionphoton, kelkkanen2023latency}. Caserman et al.\ \cite{caserman2019latency} showed that end-to-end latency above 63\,ms induces significant cybersickness symptoms in immersive VR (SSQ). Waltemate et al.\ \cite{waltemate2016impact} reported that motor performance and the perception of simultaneity degrade at approximately 75\,ms and that the sense of body ownership and agency breaks down when tracking latency exceeds approximately 125\,ms (CAVE setup with full-body avatars). Table~\ref{tab:latency-thresholds} summarises thresholds reported in the literature; these motivate the thesis goal of a tracking loop under 10\,ms so that total motion-to-photon latency remains well below the 63\,ms comfort limit.
...
63\,ms & Significant cybersickness (SSQ) & \cite{caserman2019latency} \\
75\,ms & Motor performance \& simultaneity impaired & \cite{waltemate2016impact} \\
...
\subsection{Measuring motion-to-photon latency}

To evaluate and report the latency of a tracking-and-display system, the full motion-to-photon delay must be measured in the actual hardware and software configuration. Feldstein and Ellis \cite{feldstein2021video} describe a low-cost technique using an ordinary consumer camera (e.g.\ a smartphone) and a human evaluator: the camera records both the user's real motion and the system's displayed response in the same frame, and frame-by-frame analysis yields the time difference between the real event and the displayed event. No specialised hardware or software is required, so the method can be applied to any configuration; they report measurement uncertainty below 10\,ms, with refinements potentially reducing it to approximately 1\,ms. Warburton et al.\ \cite{warburton2023motionphoton} developed a high-speed camera-based method (240\,fps smartphone) that co-registers real and virtual movements by matching the display's visible frame content (e.g.\ screen colour driven by the application) to the application's frame timeline, then detects movement onset in both real and virtual data to compute latency at movement start and across the trajectory. Their technique revealed that sudden movement onset yields higher latencies (21--42\,ms in tested VR headsets) than during steady motion, when prediction can reduce effective latency to 2--13\,ms. For the present thesis, a video-based approach is adopted: a high-frame-rate camera (e.g.\ 240\,fps smartphone) records the user and the display together; real and display movement onsets are identified and co-registered (e.g.\ via an on-screen frame indicator); latency is computed from the frame difference. This is described in the Methodology and Evaluation chapters.
...
\subsection{Camera choice for low-latency capture}

Pose estimation models typically resize input to modest resolutions (e.g.\ 256$\times$256 or 384$\times$384); thus very high sensor resolution is less important than high frame rate and low capture latency. Field of view should cover the user's full body (or at least lower body and trunk) when standing in front of the sensor. For the Architect scenario, a single frontal or slightly angled camera is sufficient. To minimise capture latency, the thesis targets industrial or machine-vision cameras with USB 3.0 (UVC or USB3 Vision), global shutter, and high frame rate (60\,fps or higher); see Table~\ref{tab:camera-options}.

\paragraph{Capture latency composition.}
The capture stage contributes: (1)~\emph{exposure}---the time the sensor collects light; (2)~\emph{readout}---transfer of pixel data off the sensor; and (3)~\emph{transfer}---delivery to the host over USB. Shorter exposure reduces motion blur; sensors with larger pixels allow shorter exposure for a given brightness. Global shutter gives a well-defined capture instant and avoids rolling-shutter artefacts. Minimising buffering (e.g.\ single-frame buffer) ensures the host uses the newest frame.

\paragraph{Monochrome vs.\ colour.}
For the lowest capture latency, monochrome sensors are preferred over RGB: they avoid Bayer demosaicing (faster readout) and have higher sensitivity, allowing shorter exposure for a given scene. Pose models expect three-channel input; a monochrome frame can be replicated to three channels (e.g.\ one memory copy or a single \texttt{COLOR\_GRAY2BGR}-style conversion) before inference without retraining. This replication adds only microseconds (negligible compared to capture and inference, which are in the millisecond range), so the net effect of choosing monochrome remains a reduction in capture latency. Where minimal latency is the priority, a monochrome industrial camera is therefore the better choice.

\paragraph{Recommended sensors.}
Industrial and machine-vision cameras (e.g.\ Edmund Optics IDS, Point Grey / Teledyne FLIR Flea3 or Blackfly S) offer global shutter, USB3 Vision, 60\,fps or higher, and larger pixel sizes (e.g.\ 4--5\,\textmu m) for low capture delay. Table~\ref{tab:camera-options} summarises options used or considered for this thesis.

\begin{table}[htbp]
\centering
\caption{Cameras suited to low-latency capture for the pose pipeline (lab use).}
\label{tab:camera-options}
\small
\begin{tabularx}{\textwidth}{@{}p{2.8cm}Xp{2.2cm}@{}}
\toprule
Camera & Interface \& key specs & Role \\
\midrule
IDS (e.g.\ U3-3060CP-M-GL), FLIR Flea3 / Blackfly S & USB3 Vision; global shutter; 60--160+\,fps; mono or colour & Preferred for minimal capture latency \\
OAK-1 Lite & USB 3; on-device inference & Optional vision module \\
\bottomrule
\end{tabularx}
\end{table}

For the present project, an industrial monochrome camera (e.g.\ IDS or FLIR with global shutter and high frame rate) is recommended for the capture stage; the pipeline accepts monochrome input by replicating to three channels before pose inference.
